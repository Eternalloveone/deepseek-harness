 ...
 * @param props.onDelete - open the browser-owned session delete confirmation.
 * @param props.onUnarchive - restore this session from the archive set.
 * @param props.drag - optional draggable-row wiring.
 * @param props.flat - omit the empty status slot in the hierarchy-free flat list.
 * @param props.t - the browser root's locale seat.
 * @returns the session row.
 */
export function SessionNodeItem({ node, currentId, now, onOpen, onRename, onFork, onArchive, onDelete, onUnarchive, drag, flat = false, t }: {
  node: SessionNode
  currentId: string | undefined
  now: number
  ...
  /** Restore this session from the archive set (archived-view action). */
  ...
  const sessionMenuItems = [
    ...
    // Delete removes the persisted log, so it is styled as destructive and goes
    // through the browser-owned confirmation dialog.
    { id: 'delete', label: t('menu.deleteSession'), icon: <IconTrashOutline16 />, danger: true },
  ]
  const archivedMenuItems = onUnarchive !== undefined
      ...
      {!row.blank && (
        <span className={css.rowActions}>
          <Menu
            open={menuOpen}
            onClose={() => { setMenuOpen(false) }}
            items={onUnarchive !== undefined ? archivedMenuItems : sessionMenuItems}
            onSelect={(id) => {
              setMenuOpen(false)
