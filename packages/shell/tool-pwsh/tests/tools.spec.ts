...
describe('sandbox escalation through ctx.approval', () => {
  ...
  it('grants a same-mode sandbox_permissions request without prompting (a no-op, even without a justification)', async () => {
    ...
    const withReason = await call(ctx, 'pwsh', escalate, sandboxAgent('workspace-write'))
    expect(withReason.isError).toBe(false)
    // Same mode without a justification: pairing validation is skipped too.
    const bare = await call(ctx, 'pwsh', { command: 'Write-Output ok', description: 'bare same-mode declaration', sandbox_permissions: 'workspace-write' }, sandboxAgent('workspace-write'))
